...
def save_mask():
    if isinstance(result_mask[0], dict):
        mask_to_save = np.stack([r['segmentation'] for r in result_mask], axis=0)
    else:
        mask_to_save = np.stack(result_mask, axis=0)
