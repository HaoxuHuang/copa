...
class Visualizer:
    ...
    def draw_binary_mask_with_number(
        self, binary_mask, color=None, *, edge_color=None, text=None, label_mode='1', alpha=0.1, anno_mode=['Mask'], area_threshold=10, fontsize=None
    ):
        """
        Args:
            binary_mask (ndarray): numpy array of shape (H, W), where H is the image height and
        ...
        if text is not None and has_valid_segment:
            ...
            self._draw_number_in_mask(binary_mask, text, lighter_color, label_mode, fontsize=fontsize)
        return self.output
    ...
    def _draw_number_in_mask(self, binary_mask, text, color, label_mode='1', fontsize=None):
        """
        ...
        if label_mode == 'a':
            ...
            text = text
        if text == '11':
            self.draw_text(text, (coords_x[len(coords_x)//2] + 2, coords_y[len(coords_y)//2] - 280), color=color, font_size=fontsize)
        elif text == '10':
            self.draw_text(text, (coords_x[len(coords_x)//2] + 2, coords_y[len(coords_y)//2] - 200), color=color, font_size=fontsize)
        elif text == '5':
            self.draw_text(text, (coords_x[len(coords_x)//2] - 500, coords_y[len(coords_y)//2]), color=color, font_size=fontsize)
        elif text == '4':
            self.draw_text(text, (coords_x[len(coords_x)//2] + 100, coords_y[len(coords_y)//2] - 100), color=color, font_size=fontsize)
        elif text == '3':
            self.draw_text(text, (coords_x[len(coords_x)//2] + 100, coords_y[len(coords_y)//2] - 100), color=color, font_size=fontsize)
        elif text == '7':
            self.draw_text(text, (coords_x[len(coords_x)//2] + 20, coords_y[len(coords_y)//2] - 100), color=color, font_size=fontsize)
        else: 
            self.draw_text(text, (coords_x[len(coords_x)//2] + 2, coords_y[len(coords_y)//2] - 6), color=color, font_size=fontsize)
