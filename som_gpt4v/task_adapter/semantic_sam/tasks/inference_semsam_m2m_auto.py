...
def inference_semsam_m2m_auto_remove_robot_arm(model, image, robot_arm_mask, interleave_ratio, if_remove_robot_arm, level, all_classes, all_parts, thresh, text_size, hole_scale, island_scale, semantic, refimg=None, reftxt=None, audio_pth=None, video_pth=None, label_mode='1', alpha=0.1, anno_mode=['Mask']):
    t = []
    t.append(transforms.Resize((text_size, text_size * image.width // image.height), interpolation=Image.BICUBIC))
    transform1 = transforms.Compose(t)
    image_ori = transform1(image)
    ...
    if if_remove_robot_arm:
        robot_arm_area_index = []
        for i in range(len(sorted_anns)):
            try:
                area_interleave_ratio = (sorted_anns[i]['segmentation'] * robot_arm_mask).sum() / sorted_anns[i]['segmentation'].sum()
            except:
                import pdb; pdb.set_trace()
            if area_interleave_ratio > interleave_ratio:
                robot_arm_area_index.append(i)
    
    label = 1
    mask_map = np.zeros(image_ori.shape, dtype=np.uint8)    
    for i, ann in enumerate(sorted_anns):
        if if_remove_robot_arm and i in robot_arm_area_index:
            continue
        new_sorted_anns.append(ann)
        mask = ann['segmentation']
        demo = visual.draw_binary_mask_with_number(mask, text=str(label), label_mode=label_mode, alpha=alpha, anno_mode=anno_mode)
        mask_map[mask == 1] = label
        label += 1
    im = demo.get_image()
    return im, new_sorted_anns
