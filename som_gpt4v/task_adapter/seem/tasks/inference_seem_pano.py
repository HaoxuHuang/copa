...
def inference_seem_pano_remove_robot_arm(model, image, robot_arm_mask, interleave_ratio, if_remove_robot_arm, text_size, label_mode='1', alpha=0.1, anno_mode=['Mask']):
    t = []
    t.append(transforms.Resize(int(text_size), interpolation=Image.BICUBIC))
    transform1 = transforms.Compose(t)
    image_ori = transform1(image)
    ...
    if if_remove_robot_arm:
        ...
        for i in range(len(sorted_anns)):
            interleave_ratio = (sorted_anns[i]['segmentation'] * robot_arm_mask).sum() / sorted_anns[i]['segmentation'].sum()
            if interleave_ratio > 0.2:
