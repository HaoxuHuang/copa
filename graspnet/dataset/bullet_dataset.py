...
class BulletDataset(Dataset):
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
                 remove_outlier=False, remove_invisible=True, augment=False, load_label=False):
        ...
        self.scenename = []
        self.frameid = []
        for img_num in range(1, len(os.listdir(os.path.join(root, 'scenes', 'rgb')))//2 + 1):
            self.colorpath.append(os.path.join(root, 'scenes', 'rgb', str(img_num).zfill(4)+'.png'))
            self.depthpath.append(os.path.join(root, 'scenes', 'depth', str(img_num).zfill(4)+'.mat'))
            self.metapath.append(os.path.join(root, 'scenes', 'meta', str(img_num).zfill(4)+'.mat'))
            self.labelpath.append(os.path.join(root, 'scenes', 'mask', str(img_num).zfill(4)+'.mat'))
            x = 'scene_{}'.format(str(img_num).zfill(4))
    ...
    def get_data(self, index, return_raw_cloud=False):
        ...
        depth = scio.loadmat(self.depthpath[index])['A']
        meta = scio.loadmat(self.metapath[index])
        seg = scio.loadmat(self.labelpath[index])['A']
        ...
        # get valid points
        depth_mask = (depth > 0)
        seg_mask = seg - 1  # remove floor(lable 0) and desk(label 1)
        if self.remove_outlier:
            camera_poses = np.load(os.path.join(self.root, 'scenes', scene, self.camera, 'camera_poses.npy'))
            align_mat = np.load(os.path.join(self.root, 'scenes', scene, self.camera, 'cam0_wrt_table.npy'))
            trans = np.dot(align_mat, camera_poses[self.frameid[index]])
            workspace_mask = get_workspace_mask(cloud, seg_mask, trans=trans, organized=True, outlier=0.02)
        ...
        # ret_dict['intrinsic'] = intrinsic
        # ret_dict['factor_depth'] = factor_depth

        return ret_dict

    def get_data_label(self, index):
        '''
        Generate point cloud from depth map and color map.
        Get object pose and camera intrinsics from metadata.
        Get object label from segmentation.
        Get ground-truth grasp from grasp labels.
        '''
        color = np.array(Image.open(self.colorpath[index]), dtype=np.float32) / 255.0
...
if __name__ == "__main__":
    ...
    test_dataset = BulletDataset(root, None, None, split='test', remove_outlier=False, remove_invisible=False, num_points=20000)
    print(len(test_dataset))

    end_points = test_dataset[12]
