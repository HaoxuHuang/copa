...
class PointCloudGenerator:
    ...
    SAVE_PATH = Path('/root/data/dataset/')
    USE_FILTER = True
    ...
    def __init__(self) -> None:
        ...
        self.generating = False # flag to indicate if point cloud is being generated
    ...
    def generate_point_cloud(self, file_name, drop_nan=True):
        # start receiving data
        self.generating = True
        rospy.loginfo('Start receiving data...')
        # wait for data to be ready
        rospy.sleep(2)
        # stop receiving data
        self.generating = False
        rospy.loginfo('Stop receiving data...')
        ...
        # save pointcloud
        file_prefix = f"{file_name}{time.strftime('%H-%M-%S')}"
        pc_path = self.SAVE_PATH / 'pointcloud' / (f"{file_prefix}.npy")
        np.save(pc_path, point_cloud_with_color)
        rospy.loginfo('Point cloud saved to {}'.format(pc_path))
        # save color image
        cv2.imwrite(str(self.SAVE_PATH / 'color' / f'{file_prefix}_color1.png'), cv2.cvtColor(self.color_image1, cv2.COLOR_RGB2BGR))
        cv2.imwrite(str(self.SAVE_PATH / 'color' / f'{file_prefix}_color2.png'), cv2.cvtColor(self.color_image2, cv2.COLOR_RGB2BGR))
        # save depth image
        np.save(self.SAVE_PATH / 'depth' / f'{file_prefix}_depth1.npy', self.depth_image1)
        np.save(self.SAVE_PATH / 'depth' / f'{file_prefix}_depth2.npy', self.depth_image2)
        # reset
        self.depth_image1 = None
        self.depth_image2 = None
        self.color_image1 = None
    ...
    def filter_point_cloud(self, point_cloud_with_color, num_points=15, radius=0.003):
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        file_name = input('Enter file name to generate point cloud [default: snapshot]: ')
        if len(file_name) == 0:
            file_name = 'snapshot'
        point_cloud_generator.generate_point_cloud(file_name)